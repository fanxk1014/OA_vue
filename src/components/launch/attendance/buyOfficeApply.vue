...
<script>
  ...
  var BuyOfficeApplyPage = Vue.extend({})
  Vue.component('buyOfficeApply-page', BuyOfficeApplyPage)

  export default {
    name: 'BuyOfficeApplyPage',
    components: {
    ...
    mounted () {
      this.$emit('sendTitleToParent','办公用品购买申请');
      this.axios({
        url: this.baseUrl+'/wechat/assetBuy/modAssetBuy?businessType=2',
        method: 'get'
      }).then((response) => {
