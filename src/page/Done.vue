  ...
  <div>
    <div class="fixed-part">
      ...
      <search class="fixed-search"
              ...
              ref="search"></search>
      <tab class="fixed-tab">
        <tab-item selected @on-item-click="changeTab('合同')">合同</tab-item>
        <tab-item @on-item-click="changeTab('行政')">行政</tab-item>
        <tab-item @on-item-click="changeTab('财务')">财务</tab-item>
        <tab-item @on-item-click="changeTab('业务')">业务</tab-item>
        <tab-item @on-item-click="changeTab('人力')">人力</tab-item>
        <tab-item @on-item-click="changeTab('其他')">其他</tab-item>
      </tab>
    </div>
    <div class="list-part">
      <div class="msg-items" v-for="item in tabList">
        ...
        <div class="item-detail">
          <router-link :to="{path:'donePages',query: {id: item.taskId,stepId: item.stepId,flowId: item.flowId,tag: tabName,title: item.flowName}}">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      changeTab (v) { // 已办事宜-切换tab
        if (v === '合同') { // 合同
          this.tabName = '合同';
          this.showTodo('CONTRACT');
          this.$store.state.todoTabType = 'CONTRACT';// 回退到已办页时使用
        } else if (v === '行政') { // 行政
          this.tabName = '行政';
      ...
      showTodo(tag){// 加载已办列表数据
        if(tag){
          this.$vux.loading.show({
            text: '加载中'
          });
          this.axios({
            url: this.baseUrl+'/wechat/todo/personalFinish?tag='+tag,// 打开已办页，默认显示‘合同’
            method: 'get',
            headers: {
